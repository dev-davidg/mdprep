    ...
    <Shell title="Výber režimu">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {[5,10,20,30,50].map(n=>(
            <button key={n} onClick={()=>setCount(n)} className={`count-chip px-4 py-2 rounded-full border text-sm font-semibold hover:border-blue-300 ${count===n?'border-blue-600 bg-blue-50':''}`}>
              {n}
            </button>
          ))}
      ...
      <div className="flex items-center justify-end">
        <button onClick={go} disabled={!mode || !count}
          className={`rounded-full text-white text-sm font-bold px-6 py-3 ${mode&&count?'bg-blue-500 hover:bg-blue-600':'bg-blue-500/50 cursor-not-allowed'}`}>
          Pokračovať
        </button>
      </div>
    </Shell>
